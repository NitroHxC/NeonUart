...
#define NEON_MSG_IN(NAME)  NAME##_ID, sizeof(NAME), cb_##NAME
#define NEON_MSG_OUT(NAME) NAME##_ID, sizeof(NAME), 0
...
typedef void (*handler_callback_t)(neon_parser_t* pParser, uint8_t txrx, uint8_t type,
                                   uint8_t* payload, uint8_t* output);
...
// TX
void neon_define_message(neon_parser_t* pParser, uint8_t msg_type, uint16_t length,
                         app_callback_t cb);
uint8_t neon_build_message(neon_parser_t* pParser, uint8_t* pay, uint8_t type, uint16_t paylen,
                           uint8_t* msg_out);
