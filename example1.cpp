...
{
    msg1_t msg = { 0 };
    memcpy(&msg, pv, sizeof(msg1_t)); // Can copy data here or to a global. After going out of the callback scope, the data will be freed
...
{
    msg2_t msg = { 0 };
    memcpy(&msg, pv, sizeof(msg2_t)); // Can copy data here or to a global. After going out of the callback scope, the data will be freed
...
void example1()
{
    parser_ctx_t parser1 = parser_ctx_t(); // create Parser object

    /* 1. set callback for printing an unhandled Msg ID when received (if needed) */
    uart_parser_set_unhandled_cb(&parser1, print_unhandled);

    /* 2. define messages dynamically:
    ...
    uart_define_message(&parser1, MSG1_ID, sizeof(msg1_t), callback_msg1);

    uart_define_message(&parser1, MSG2_ID, sizeof(msg2_t), callback_msg2);
    ...
    /* 4. Setup is done, enjoy! */

    /* ------------------------ */
    
    /* 5. Test 1 : send and receive msg1 */
    uint8_t buffer_out[N_MAX_PAYLOAD] = { 0 };

    msg1_t message_one = msg1_t();
    message_one.f1 = 666.6f;
    message_one.i2 = 999;
    message_one.c3 = 22;

    uint8_t len_out = uart_build_message(&parser1, (uint8_t*)&message_one, sizeof(msg1_t), 0x03, buffer_out);
    ...
    test_parse_buffer(&parser1, buffer_out, len_out);

    /* ------------------------ */

    /* 6. Test 2 : send a message with wrong ID (0x05 instead of 0x03) */
    // Let's test the unhandled callback
    memset(buffer_out, 0, N_MAX_PAYLOAD);
    len_out = uart_build_message(&parser1, (uint8_t*)&message_one, sizeof(msg1_t), 0x05, buffer_out);
    ...
    /* 7. Test 3 : send and receive msg2 */
    memset(buffer_out, 0, N_MAX_PAYLOAD);

    msg2_t message_two = msg2_t(); // in C we actually initialize with = { 0 }
    message_two.c1 = 1;
    message_two.c2 = 2;
    message_two.c3 = 3;
    ...
    len_out = uart_build_message(&parser1, (uint8_t*)&message_two, sizeof(msg2_t), MSG2_ID, buffer_out);

    print_debug(buffer_out, len_out);

    // get msg2!
    test_parse_buffer(&parser1, buffer_out, len_out+1);


    uart_parser_deinit(&parser1);
}
...
void test_parse_buffer(parser_ctx_t* parser, uint8_t* buffer, uint16_t maxlen)
{
    ...
    */
    for (uint16_t i = 0; i < maxlen; i++)
    {
        ...
        if (uart_parse_char(parser, buffer[i]))
        {
            break;
        }
    }
